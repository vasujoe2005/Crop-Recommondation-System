...
import { getAuthToken, getRecommendationHistoryStorage, saveRecommendationRecord } from '../utils/storage';
...
export const createFarm = async (payload) => (await api.post('/farms', payload)).data;
export const getFarms = async () => (await api.get('/farms')).data?.items || [];
export const getFarmById = async (farmId) => (await api.get(`/farms/${farmId}`)).data;
export const deleteFarm = async (farmId) => (await api.delete(`/farms/${farmId}`)).data;
export const searchFarmLocation = async (query) => (await api.get('/farms/search/location', { params: { query } })).data;
